...
- Доработать карточку места: владелец, статус, отдача, назначение, линия, история.
- Добавить быстрые операции из карточки: назначить сотрудника, назначить гостя, отменить назначение, создать отдачу.
- Улучшить KPI, легенду карты и фильтры по этажу/статусу/типу.
- Показывать API warnings после ручных и гостевых назначений.
- Улучшить вкладку `Заявки`: отдельные таблицы сотрудников, гостей и очереди с причинами статусов.

Acceptance:

- Полный дневной сценарий выполняется без SQL.
- Клик по месту на карте открывает карточку места.
- Ошибки и предупреждения API видны в UI.

### M3. Catalog, History, Audit

Цель: дать администратору полный контроль без прямого доступа к БД.

- Доработать CRUD сотрудников и мест.
- Доработать управление постоянными закреплениями.
- Расширить историю места и сотрудника.
- Улучшить audit filters по дате, action, entity type и actor.
- Добавить отображение последнего успешного запуска каждого job.

Acceptance:

- Любое создание, отмена, назначение и изменение справочника видно в audit/history.
- История открывается из рабочих экранов.
- Jobs имеют понятный операционный статус.
