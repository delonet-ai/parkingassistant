...
Списки, у которых SQL-сортировка неоднозначна (`/admin/audit-logs` сортируется по
`occurred_at desc` без tiebreaker, а все строки одной транзакции демо-сида имеют одну метку
времени), помечены в сценарии как `unordered`: внутри них идентичность обезличивается до
`<uuid>`, а строки сортируются по нормализованному содержимому. Утверждается множество и
содержимое строк, а не порядок, который Postgres выбрал в этот раз.
