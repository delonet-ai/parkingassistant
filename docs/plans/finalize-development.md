...
- [x] Replace `renderOperationalMap` with the static floor plan `<img>` plus `renderPlaceLines(..., { mode: 'operational' })`. Delete the function's SVG zone rendering, zone click wiring, `.map-zone-selected` handling, the client-side zone filtering loop, and the label-size heuristic (`box.width >= 36 && box.height >= 18`) — none of it has a consumer once zones are gone. — the whole panel moved into a new pure component `apps/admin-web/src/render-day-map.js` (`renderDayMap`), matching the shape Task 10 established: it takes a model and the already-rendered drawer HTML and returns escaped markup, so it is unit-testable even though `server.js` self-starts and exports nothing. `renderOperationalMap` is now a ten-line adapter. Gone with the SVG: `pixelBoxFromGeometry`, `setSvgRectAttributes`, `effectiveStatus`, `renderZone`, `loadZones`, the `map-zones-layer`, the `.operational-map-card` per-floor cards and the label-size heuristic. The dead `.operational-maps-grid` CSS rule became `.operational-places`.
- [x] Preserve the selection principle exactly: clicking a slot sets `aria-pressed`, fetches `/admin/operational-place-card`, swaps the `.place-drawer` aside without reload, and `history.pushState`s the day URL; `popstate` restores selection. Keyboard: Tab to a slot, Enter/Space selects. — the fetch/swap/pushState/popstate block is carried over verbatim; only the selection setter changed (`aria-pressed` + `.place-slot--selected` on the button, instead of a class on an SVG rect). Keyboard support is now free rather than absent: a slot is a real `<button>`, so Tab reaches it and Enter/Space fire the same click handler — the SVG `<g>` it replaces was unreachable by keyboard at all.
- [x] Keep the drawer unchanged — status, owner/assignment/release/line-position grid, "Отдать место на день", "Назначить сотрудника", "Отменить назначение", the guest-request `<details>` block, and the link to place history — including the hidden `placeId`/`date` fields that restore the day view after a redirect. Replace the `mapCode` field with the floor selector's value, keeping the existing convention that map code `g4` maps to `parking_places.floor_label` `4` — document it rather than re-deriving it ad hoc in a third place. — `renderOperationalPlaceCard` is untouched apart from its empty-state copy ("нажмите на размеченное место на карте" described a control that no longer exists). The floor conversion goes through Task 10's `mapCodeToFloorLabel()`; no third `replace(/^g/, '')` was added.
- [x] Keep the status/type filters and the legend, applied to elements instead of zones. — filtering moved from the browser to the server as the pure, exported `filterDayLines()`. Type is a line-level question because `place_type` is derived from `line_groups.capacity`; status keeps a line when **at least one** slot matches, since hiding a whole triple because its rear slot is occupied would hide the free front slot with it. The legend keeps the six colour tokens and now prints the Russian word next to each, so it reads the same way the slots do.
- [x] Integration/render tests: selecting a slot yields the same drawer payload the zone click produced; every drawer action still round-trips. — `apps/admin-web/src/render-day-map.test.js` (19 tests): the five filter rules incl. unknown-status fallback and a missing line list, the static `<img>` with no `<svg>`/`viewBox`/`map-zone`/`/admin/map-zones` anywhere, slot buttons with `aria-pressed`, exactly one selected slot, the drawer HTML passed through verbatim, the card-endpoint/pushState/popstate wiring, filter controls preselected, filters actually applied to the rendered grid (not just the controls), other floors hidden, no editor-only controls leaking onto the Day tab, escaping, and a non-blank empty state. Also rendered end-to-end against a stubbed API: `?view=day&mapCode=g4&placeId=place-1` returns 200 with the slot server-side marked `aria-pressed="true"`, `?status=blocked` hides the free element, and `/admin/operational-place-card` returns the byte-identical drawer the zone click used to open — same endpoint, same handler, same payload.
- [x] Run the validation commands; green. — `npm run check` / `lint` clean, `npm test` 103 pass (84 + 19). `render-day-map.js` was added to the `check` script. A leftover grep for `svgPoint|normalizedRect|map-zones-layer|renderZone|loadZones|pixelBoxFromGeometry|setSvgRectAttributes|effectiveStatus|operational-zone-group` over `apps/`, `packages/` and `scripts/` returns nothing.
- [x] Mark completed.
...
- [x] Write `docs/adr/003-modular-architecture.md` capturing the controller/service/repository/domain contract and the one-way dependency rule. — includes the per-layer "may import / must not contain" table, the module layout, the bounded-context list, the one-database-pattern decision, and how the boundary is enforced. Written in English, matching `docs/ARCHITECTURE.md` and the plan's language rule (ADRs 001/002 are Russian and stay as they are — an ADR is a dated record).
- [x] Write `docs/adr/004-place-inventory.md` recording the Phase 2 decision: zone geometry removed, elements are 1–3-slot lines, delete means archive. — records what actually shipped in Tasks 8–12, not just the intent: uniform element identity (a single is a capacity-1 `line_groups` row), `capacity` as the single source of truth with `assign_place_lines()` as its one implementation, `place_role` promoted out of `geometry.zoneType` **before** the drop, archive-with-named-blockers, the Карта → Места rename with no URL alias, and the selector-changed/interaction-preserved principle. Marked as superseding the map-zone part of ADR 002, which already carries the matching supersession note from Task 12.
- [x] Reconcile the two DB patterns: standardize on `repositories/` (`queryOne`/`queryMany`) and add a `withTransaction(pool, fn)` helper yielding a client-bound repository; inline `client.query` in services is not deprecated but removed outright by Task 15, which leaves no SQL outside `repository.js` files. — `apps/api/src/repositories/db.js` now exports `createDbRepository`, `createClientRepository` and `withTransaction`, all three sharing one `buildQueries` implementation so a query moves between pool-bound and client-bound without being rewritten. The helper **never inspects the return value**: the 19 existing transaction blocks in `server.js` return 404/409 payloads on their abort paths, and if "returned an error payload" implied rollback the migration in Task 15 would change behavior silently. A service that wants to abort throws. A failing rollback is attached as `error.rollbackError` rather than replacing the original error, and the client is released on every path.
- [x] Lock the layout `apps/api/src/modules/<context>/{controller,service,repository}.js` and enumerate the bounded contexts (employees, places, place-lines, permanent-assignments, place-releases, employee-requests, guest-requests, reservations, queue, line-occupancy, departure-plans, conflicts, contact-access, maps, dashboard, audit, jobs). — enumerated in ADR 003 with what each owns, and mirrored into `docs/ARCHITECTURE.md`; a test asserts the two lists agree, so they cannot drift. `availability` is explicitly **not** a context — it is a read model whose SQL moves into `place-releases` / `places` / `reservations` in Task 15. No directories created: no code moves yet.
- [x] Add ESLint `no-restricted-imports` rules: `packages/domain/**` may not import `pg`/`node:http`/`packages/shared/http|html`; `**/controller.js` may not import `pg`. — **the rule as specified enforces nothing here.** `no-restricted-imports` only sees `import` declarations, and this project is CommonJS throughout, so a config carrying only that rule would pass every violating file. Both globs therefore also carry `no-restricted-syntax` selectors over `CallExpression[callee.name="require"]` (with a regex arm for `**/shared/http|html` at any relative depth), which is what actually bites. `no-restricted-imports` is kept alongside it so the boundary survives a future move to ESM.
- [x] No code moves yet. `npm run check && npm run lint && npm test` green. — `check` gained `repositories/db.js`; `lint` clean; `npm test` **149** pass (129 + 12 for `withTransaction` + 8 boundary/module-map). `architecture.test.js` verifies the boundaries by running ESLint programmatically over synthetic sources at the target paths rather than by inspecting the config object — asserting the config shape would not have caught the CommonJS trap above.
- [x] Mark completed.
...
- [x] Move each handler group into `modules/<context>/controller.js`; controllers hold no SQL, services own transactions. — 50 handlers moved into 17 controllers. The split point is the layer, not the function: the controller keeps `readJsonBody` and its JSON error, searchParams reading and defaults, validation that needs no database, the error→status mapping and the payload construction; the service keeps every repository call and every `withTransaction` block. **The sentinel is what makes that split possible** — `AbortTransaction`/`abortWith` moved to `src/support/transaction.js`, is thrown from inside the service's transaction and caught by the controller, so a rolled-back use case still answers 404/409 with its own payload while `withTransaction` stays blind to the return value. Two rules were settled here and written into ADR 003: **a service may require any context's repository** (a transaction spans contexts by nature — forcing the ~19 of them through sibling services would have produced ~60 pass-through methods and no extra safety), and **a controller may require none**.
- [x] Replace `router.js`'s monolithic `if/else` + `rootEndpoints` list with per-module route tables the router composes; keep every URL/method/payload identical. — `router.js` is down to 128 lines and owns no route. Each controller returns `{ name, routes }`; the router builds an exact-path map plus a pattern list, and **derives the `GET /` endpoint index from the same tables** (entries marked `advertise`), so a route and its documentation cannot drift. That index is byte-identical to the hand-kept array it replaces, which constrained the composition order: `/admin/line-groups` and `/bot/line/blocking-contacts` are served by the `line-occupancy` module, which owns the whole `/admin/line-groups*` + `/bot/line/*` URL space, reading the roster through `services.placeLines` and writing its logs through `services.contactAccess` rather than reaching into either repository. The `safe` flag per route preserves exactly which endpoints used the catch-all dispatcher.
- [x] Reduce `apps/api/src/server.js` to a thin bootstrap. — **4 960 → 65 lines**: env, pool, `createModules`, `createApiRouter`, listen, shutdown. `modules/index.js` (111 lines) is the composition root: it builds every service into one registry and hands that registry to every factory, so mutually dependent contexts (jobs → queue, guest-requests → reservations) resolve a sibling at call time instead of needing a load order. `queue` is the one context with a service and no controller — it serves no route and runs only under the process-queue job.
- [x] Golden + integration tests unchanged; all validation commands green. — **not one of the 122 golden snapshots was regenerated**, verified byte-identical over four consecutive full runs against a live Postgres; `npm run test:integration` **266/266**. `npm run check` / `lint` clean, `npm test` **229** (222 + 7 new). `apps/api/src/module-boundary.test.js` asserts the layer rule repo-wide (no controller requires a repository, `pg`, `repositories/db` or `services/availability`), that every controller has a service, and — so it fails without a database — that the composed route tables reproduce the published endpoint index in order and register every method/path the monolith served.
- [x] Mark completed.

**`npm run check` is no longer a hand-kept list.** It was 40-odd `node --check <file>` calls that had to be extended by hand in Tasks 13, 15 and 16; this task alone adds thirty-odd files. `scripts/check-syntax.js` walks the tree instead and parses all 131 files, so a new module is covered the moment it is written rather than the moment someone remembers.

**One asymmetry preserved rather than tidied up:** only `/admin/jobs/process-queue` honours `error.statusCode` (the 409 the queue stamps on a `23505`); the other four jobs answer 500 even when the error carries a status. The shared failure helper takes the status explicitly so the difference survives instead of being accidentally unified. Likewise `/admin/jobs/runs` keeps its inline `Math.min(Math.max(Number(...) || 20, 1), 100)` rather than adopting `parsePositiveLimit`, which differs on `limit=-5` and on non-numeric input.

### Task 18: Turn on and verify the boundaries
- [x] Enable the dependency-direction ESLint rules repo-wide; fix all violations. — Task 13 shipped the rules on two globs (`packages/domain/**`, `**/controller.js`); they now cover every layer of the tree. Added: `packages/shared/**` as a leaf (no `pg`, no reaching up into an application); `**/service.js` **and** `apps/api/src/services/*.js` (no `pg` — transactions go through `withTransaction` — no `node:http(s)`, no `shared/http|html`, no controller); `**/repository.js` (no HTTP, no HTML, no service, no controller); and on the existing two, no requiring an app or `packages/db` from domain, and no reaching past the service to a `repository`/`repositories/db`/`services/availability` from a controller. **There were zero violations to fix** — Tasks 15–17 landed the tree already obeying the direction, which is the outcome the ordering was for. Each rule only forbids what points the wrong way: a service requiring any context's repository stays legal, because a transaction spans contexts.
- [x] Verify no controller contains raw SQL and no domain module imports pg/http; add a check asserting it. — three checks in `architecture.test.js`, and **each was verified to fail by planting a violation** rather than assumed: (1) the synthetic-source lint from Task 13, extended to the new layers, proving every rule fires — the CommonJS trap means a config that *looks* right can enforce nothing; (2) a lint of the **real** tree asserting zero boundary messages, so `npm test` fails on a violation even if nobody runs `npm run lint`; (3) a direct source scan of the 17 controllers for SQL statements and of the domain modules for SQL or a forbidden `require` — **inline SQL is invisible to any import rule**, so the lint alone would not have satisfied this checkbox.
- [x] Remove orphaned code from the former monolith; confirm no dead exports. — no orphaned files: every non-entrypoint module in `apps/`, `packages/` and `scripts/` is required by something, and `server.js` has been a 65-line bootstrap since Task 17. Sixteen dead *exports* did survive the extraction, all names whose caller no longer exists: `lineCodeFor`/`lineNameFor`/`GUEST_PRIORITY_RANK_MIN`/`MAX` in `packages/domain/line-inventory.js` (now internal to `buildLineDefinition`), `migrateDatabase`, four in `seed-demo.js`, two in the test harness, two in `golden.js` and three fixture ids in `stub-api.js`. All removed per the no-compatibility rule. `dead-exports.test.js` now asserts it repo-wide so the next extraction cannot re-accumulate them; it too was verified to fail on a planted export, and it carries a vacuity guard so a broken walker fails instead of passing.
- [x] `npm run check && npm run lint && npm test && npm run test:integration` all green. — `check` 132 files, `lint` clean, `npm test` **241** (229 + 10 boundary/purity + 2 dead-export), `npm run test:integration` **266/266** against a live Postgres, stable over 3 consecutive full runs. **Not one of the 122 golden snapshots was regenerated** — the HTTP contract is untouched, as a task that only adds checks should leave it. Docs updated: ADR 003's Enforcement section now lists the per-layer rules and the four checks that hold them, and `docs/ARCHITECTURE.md` carries the same in short form.
- [x] Mark completed.

**The integration run needs a Postgres this dev container does not have** (no docker, no local
server). It was run against a throwaway `postgres:16-bookworm` container started on the OMV host on
port `5433` with a tmpfs data directory, then stopped and removed — deliberately *not* the
`parkingassistant-postgres` stand database, even though the harness only ever creates and drops its
